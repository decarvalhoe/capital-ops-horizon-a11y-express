...
// mini-DOM: enough for the selectors used
...
test('compliant page → no findings', () => {
...
test('failing page → expected findings', () => {
...
test('ids with quote, bracket, backslash: no crash, labels recognised, diagnosis kept', () => {
  const ids = ['a"b', 'c[d]', 'e\\f', "g'h"];
  const nodes = [el('h1', {}, 'T'), el('main', {})];
  ids.forEach((id) => { nodes.push(el('label', { for: id }, 'L')); nodes.push(el('input', { id })); });
  nodes.push(el('input', { id: 'sans"label' }));
  const r = runChecks(doc(nodes, 'fr'));
  const label = r.find((x) => x.id === 'label');
  assert.ok(label && label.count === 1, JSON.stringify(r));   // only the unlabeled field is reported
});
test('DOM labels property takes precedence when available', () => {
  const inp = el('input', { id: 'x' }); inp.labels = [{}];
  const r = runChecks(doc([el('h1', {}, 'T'), el('main', {}), inp], 'fr'));
  assert.equal(r.find((x) => x.id === 'label'), undefined);
});
test('messages are English by default (global audience)', () => {
  const d = doc([el('h2', {}, 'x'), el('img', {}), el('input', { id: 'a' })], '');
  for (const f of runChecks(d, null)) assert.doesNotMatch(f.msg, /[éèàç]/, f.msg);
});
test('bookmarklet escapes finding messages before inserting them in the panel (a message containing <h1> must not become markup)', async () => {
  const fs = await import('node:fs'); const b = fs.readFileSync(new URL('./bookmarklet.js', import.meta.url), 'utf8');
  assert.ok(b.includes("esc(x.msg)") && b.includes("replace(/</g,'&lt;')"));
});
